...
class LLaMA(nn.Module):
    ...
    def forward(
        ...
        if input_pos is not None:
            ...
            assert torch.all(input_pos < self.rope_cache.size(0)), f"Can't select the {input_pos} index bigger than {self.rope_cache.size(0)}" # 确保索引小于维度0的大小
            rope = self.rope_cache.index_select(0, input_pos) # out of range in self
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is not None:
            ...
            # print("test-att1", input_pos[-1], max_seq_length, input_pos[-1] >= max_seq_length)
