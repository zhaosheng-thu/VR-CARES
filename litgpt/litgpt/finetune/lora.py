...
def fit(
    ...
    while step_count < max_steps and train_iterator.epoch < train.epochs:
        ...
        if not is_accumulating and step_count % eval.interval == 0:
            ...
            # for esconv
            generate_example_esconv(fabric, model, tokenizer, eval, data)
...
@torch.no_grad()
def generate_example_esconv(fabric: L.Fabric, model: GPT, tokenizer: Tokenizer, eval: EvalArgs, data: DataModule):
    dialogue_content = {}
    dialogue_content['scene'] = "Communication Challenges"
    dialogue_content['description'] = 'I recently moved to a new city for a job and finding it hard to make friends.'
    dialogue_content['User'] = (
        "I'm doing okay, but I'm feeling a bit lonely lately. I recently moved to a new city for a job,"
        "and it's been hard for me to make friends. I miss having a close-knit group of friends like I did in my hometown."
        )
    prompt_scene = data.prompt_style.apply(prompt='scene', **dialogue_content)
    prompt_description = data.prompt_style.apply(prompt='description', **dialogue_content)
    prompt_user = data.prompt_style.apply(prompt='User', **dialogue_content)
    prompt = prompt_scene + prompt_description + prompt_user + f"<|start_header_id|>Assistant Strategy<|end_header_id|>\n\n"
    fabric.print(prompt)
    encoded = tokenizer.encode(prompt, device=fabric.device)
    model.eval()
