    ...
    def apply(self, prompt: str, **kwargs: str) -> str:
        ...
        if prompt == "scene":
            return (
                ...
                f"Now there is a {kwargs['scene']} scene.<|eot_id|>\n"
            ) if kwargs.get("scene") else (
                "<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\n"
                "You are a helpful assistant designed for emotion support task. "
                "Now there is a scene<|eot_id|>\n"
            )
        elif prompt == "description":
            return (
                "<|start_header_id|>User's background<|end_header_id|>\n\n"
                f"{kwargs['description']}<|eot_id|>\n"
            ) if kwargs.get("description") else (
                "<|start_header_id|>User's background<|end_header_id|>\n\n"
                "The user's background is unknown.<|eot_id|>\n"
            )
        elif prompt == "User":
            return (
                "<|start_header_id|>User<|end_header_id|>\n\n"
                f"{kwargs['User']}<|eot_id|>\n"
            )
        elif prompt == "AI Strategy":
            return (
                "<|start_header_id|>Assistant Strategy<|end_header_id|>\n\n"
                f"{kwargs['AI Strategy']}<|eot_id|>\n"
            )
        elif prompt == "AI":
            return (
...
# TODO: add more prompt styles here, self-designed style of the emotion support conv aug datasets:
class Llama3ESConv(PromptStyle):
    def apply(self, prompt: str, **kwargs: str) -> str:
        return f""
...
prompt_styles: Dict[str, Type[PromptStyle]] = {
    ...
    'default': Default,  # 'default' is a special case that doesn't apply any prompt style
    "alpaca": Alpaca,
    "flan": FLAN,
    "longform": Longform,
